...
def encode_samples(tables, embeddings, table2vec):
    samples_enc = []
    for i, query in enumerate(tables):
        if len(query) != len(embeddings[i]):
            raise ValueError("Number of tables and embeddings do not match for query {}".format(i))
        samples_enc.append(list())
        ...
        for j, table in enumerate(query):
            ...
            sample_vec.append(embeddings[i][j])
            sample_vec = np.hstack(sample_vec)
            samples_enc[i].append(sample_vec)
    return samples_enc
